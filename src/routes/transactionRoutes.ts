...
router.get(
  ...
  > => {
    ...
    //add pagelimit and offset to the query if provided
    if (!isNaN(Number(pageLimit))) {
      const start = Number(offset) ?? 0;
      const end = start + Number(pageLimit) - 1;
      supabaseQuery = supabaseQuery.range(start, end);
...
router.post(
  ...
  async (req: MerchantAuthRequest, res: Response) => {
    ...
    const { data: virtualAccData, error: virtualAccError } = await supabase
    ...
    if (paymentMethods.length === 0) {
      return res.status(400).json({
        data: null,
        error:
          "No payment methods available for this currency. Kindly try again later.",
      });
    }
...
router.post(
  ...
  async (req: MerchantAuthRequest, res: Response) => {
    ...
    }: { data: TransactionType | null; error: PostgrestError | null } =
    ...
    const { data: paymentMethodData, error: paymentMethodDataErr } =
      await supabase
        .from("payment_methods")
        .select("*")
        .eq("id", payload.payment_method_id)
        .eq("name", payload.tx_type)
        .single();
    ...
    if (!paymentMethodData.available) {
      return res.status(400).json({
        data: null,
        error:
          "Payment Method is not available currently. Kindly try again later.",
      });
    }
    ...
    if (paymentMethodData.minimum_amount > Number(payload.amount)) {
      return res.status(400).json({
        data: null,
        error:
          "Amount is less than the minimum amount for this payment method.",
      });
    }
    ...
    if (paymentMethodData.maximum_amount < Number(payload.amount)) {
      return res.status(400).json({
        data: null,
        error:
          "Amount is greater than the maximum amount for this payment method.",
      });
    }
    ...
    const feeRate =
      paymentMethodData.fee_type === "percentage"
        ? paymentMethodData.fee_rate
        : paymentMethodData.fee_type === "flat"
        ? paymentMethodData.fee_rate
        : 0;
    const status =
      txType === "card"
        ? "pending"
        : txType === "virtual_account"
        ? "success"
        : "pending";
    const feeAmount = roundToTwo(
      paymentMethodData.fee_type === "percentage"
        ? (paymentMethodData.fee_rate * amount) / 100
        : paymentMethodData.fee_type === "flat"
        ? paymentMethodData.fee_amount
        : 0
    );
    ...
    if (txType === "card") {
      const sanitizedCardNumber = (payload.card_number ?? "").replace(
        /\s+/g,
        ""
      );
      ...
      Object.assign(baseTransactionRecord, {
        ...
        card_verification_code: null,
        customer_name:
          payload.customer_name ?? payload.customer_account_name ?? null,
        customer_phone_number: payload.customer_phone_number ?? null,
    ...
    if (merchantBalanceError || !merchantBalances) {
      ...
      return res.status(500).json({
        data: null,
        error:
          merchantBalanceError?.message || "Failed to fetch merchant balances.",
      });
    }
    ...
    const updatedBalances = {
      ...
      pending_settlement_balance:
        txType === "card"
          ? roundToTwo(
              (merchantBalances.pending_settlement_balance ?? 0) + netAmount
            )
...
router.patch(
  ...
  async (req: MerchantAuthRequest, res: Response) => {
    ...
    if (
      transactionData.card_last_four_digits !== payload.card_number.slice(-4)
    ) {
      return res.status(400).json({
        data: null,
    ...
    const { error: balanceUpdateError } = await supabase
      .from("merchant_balance")
      .update({
        available_balance: roundToTwo(
          (merchantBalances.available_balance ?? 0) +
            transactionData.total_amount
        ),
        pending_settlement_balance: roundToTwo(
          (merchantBalances.pending_settlement_balance ?? 0) -
            transactionData.total_amount
        ),
      })
      .eq("merchant_id", transactionData.merchant_id)
